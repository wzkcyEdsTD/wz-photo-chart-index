...
const fcmain = function(window) {
  ...
  var support = {
    ...
    selectUl = document.querySelector(".unusedBuilding"),
    selectUlLiEl = selectUl.querySelector(".unusedFloor"),
    selectUlLis = [].slice.call(selectUl.querySelectorAll(".unusedFloor")),
    // the mall element
    ...
    spacesList = new window.List("spaces-list", {
      valueNames: [
        "list__link",
        {
          data: ["level"]
  ...
  function initEvents() {
    ...
    mallLevels.forEach(function(level) {
      level.addEventListener("click", function(evt) {
        evt.stopPropagation();
        const pos = parseInt(evt.currentTarget.attributes["data-key"].value);
        // shows this level
        showLevel(pos, true);
      });
    });

    selectUlLis.forEach(function(level) {
      level.addEventListener("click", function(evt) {
        evt.stopPropagation();
        const pos = parseInt(evt.currentTarget.attributes["data-val"].value);
        showAllLevels(true, pos, () => {
          showLevel(pos, true);
        });
        // shows this level
      });
   ...
   */
  function showLevel(level, isList) {
    if (isExpanded) {
      return false;
    }
    ...
    if (isList) {
      mallLevels.map(item => {
        if (item.attributes["aria-label"].value == `Level ${level}`) {
          levelEl = item;
        }
      });
    }
   ...
   */
  function showAllLevels(tag, pos, fn) {
    if (pos && pos == selectedLevel) return;
    if (tag) isExpanded = tag;
    if (isNavigating || !isExpanded) {
      return false;
    }
    isExpanded = false;
    let forceLevel = mallLevels[selectedLevel ? selectedLevel - 1 : 0];
    mallLevels.map(item => {
      if (item.attributes["aria-label"].value == `Level ${selectedLevel}`) {
        forceLevel = item;
      }
    });
    classie.remove(forceLevel, "level--current");
  ...
  function _showAllLevels(tag, pos) {
    if (pos && pos == selectedLevel) return;
    if (tag) isExpanded = tag;
    if (isNavigating || !isExpanded) {
      return false;
  ...
  function showPins(levelEl) {
    ...
    levelEl.querySelector(".level__pins") && classie.add(levelEl.querySelector(".level__pins"), "level__pins--active");
    levelEl.querySelector(".level__pins a div") && classie.add(
      levelEl.querySelector(".level__pins a div"),
      "level__pins--active"
    );
  }
  ...
  function removePins(levelEl) {
    var levelEl = levelEl || mallLevels[selectedLevel ? selectedLevel - 1 : 0];
    mallLevels.map(item => {
      if (item.attributes["aria-label"].value == `Level ${selectedLevel}`) {
        levelEl = item;
      }
  ...
  function navigate(direction) {
    ...
    let _forceLevel, _preLevel, _nextLevel;
    mallLevels.map((item, index) => {
      if (item.attributes["aria-label"].value == `Level ${prevSelectedLevel}`) {
        currentLevel = item;
        _preLevel = mallLevels[index - 1];
        _nextLevel = mallLevels[index + 1];
      }
    });

    if (direction === "Up" && prevSelectedLevel > 1) {
      _forceLevel = _preLevel;
    } else if (direction === "Down" && prevSelectedLevel < mallLevelsTotal) {
      _forceLevel = _nextLevel;
    } else {
      isNavigating = false;
      return false;
    }

    selectedLevel = _forceLevel.attributes["data-key"].value;
    ...
    classie.add(_forceLevel, "level--current");
    ...
    onEndTransition(currentLevel, function() {
      ...
      classie.remove(currentLevel, "levels--selected-" + prevSelectedLevel);
      classie.add(_forceLevel, "levels--selected-" + selectedLevel);

      // show the current level´s pins
      showPins(_forceLevel);
   ...
   */
  function openContent(spacerefval, level) {
    ...
    classie.add(
      spacesEl.querySelector('li[data-space="' + spacerefval + '"]'),
      "list__item--active"
    );
    var levelEl = mallLevels[selectedLevel - 1];
    mallLevels.map(item => {
      if (item.attributes["data-key"].value == level) {
        levelEl = item;
      }
    });

    // remove class selected (if any) from current space
    var activeSpaceArea = levelEl.querySelector("svg > .map__space--selected");
    if (activeSpaceArea) {
      classie.remove(activeSpaceArea, "map__space--selected");
    }
    // svg area gets selected
    classie.add(
      levelEl.querySelector('svg > .map__space[data-space="' + spaceref + '"]'),
      "map__space--selected"
    );
  }
